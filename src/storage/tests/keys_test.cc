...
std::unique_ptr<PikaConf> g_config;
...
static bool key_value_match(const std::vector<storage::KeyValue>& key_value_out,
  ...
  if (key_value_out.size() != expect_key_value.size()) {
    LOG(WARNING) << "key_value_out.size: " << key_value_out.size()
                 << " expect_key_value.size: " << expect_key_value.size();
    return false;
  }
  for (int32_t idx = 0; idx < key_value_out.size(); ++idx) {
    LOG(WARNING) << "key_value_out[idx]: " << key_value_out[idx].key
                 << " expect_key_value[idx]: " << expect_key_value[idx].key;
    LOG(WARNING) << "key_value_out[idx]: " << key_value_out[idx].value
                 << " expect_key_value[idx]: " << expect_key_value[idx].value;
...
int main(int argc, char** argv) {
  ...
  g_config = std::make_unique<PikaConf>(pika_conf_path);
  if (g_config.Load()) {
    printf("pika load conf error\n");
    return 0;
  }
  if (!pstd::FileExists(g_config.log_path())) {
    pstd::CreatePath(g_config.log_path());
  }
  FLAGS_log_dir = g_config.log_path();
  FLAGS_minloglevel = 0;
